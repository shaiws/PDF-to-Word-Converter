from os import environ, path,pathsep, remove, name,startfile
...
from tkinter import filedialog, simpledialog, ttk, Tk
...
def create_docx_from_images(images, docx_path, width, progress=None, root=None):
    ...
    for i in tqdm(range(len(images)), desc='Converting images to Word document', unit='image'):
        ...
        remove(img_filename)  # Remove image file after using
        if progress:
...
def open_file(path):
    ...
    if name == 'nt':  # for windows
        ...
        opener = "open" if name == "posix" else "xdg-open"  # for mac and linux respectively
        call([opener, path])
...
def main():
    ...
    """
    parser = ArgumentParser(description='Convert a PDF to a Word document with each page as an image.')
